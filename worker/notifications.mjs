...
export function notificationMessage(kind) {
  if (kind === 'launch') return {
    ...
    text:`The First Then Board Android beta is now available. Thanks for helping us test it! Your Google account has been added to our tester group.\n\nTo install on your Android phone or tablet:\n1. Open ${PLAY_URL}\n2. Sign in with the same Google account you used on the signup form. Check that the Play Store also uses this account.\n3. Select Become a tester. Signing up on our website alone does not complete this step.\n4. Follow the download link to Google Play and select Install. Open First Then Board when installation finishes. You can install on both a phone and tablet using the enrolled account.\n\nThe beta supports Android 8 or newer. International availability is expanding and may depend on Google Play review and rollout. If Google says the app is unavailable, first check the signed-in account, then allow time for group access to update and reopen the invitation. If you previously joined our internal test, leave that test before joining this closed beta. If installation fails or an older test app is already installed, reply for help before uninstalling so we can protect your saved boards and photos.\n\nPlease stay opted in for at least 14 days and use the app regularly during your usual routines. Try creating a board, choosing photos, changing the layout, and reopening the app. Participation is voluntary; you can leave at any time. The free features are enough to help test, and no Premium purchase is required.\n\nFor feedback, use Send feedback in the app menu or reply to this email. Tell us your device model, what you tried, and what happened. Please do not include a child's name, photos, or medical information. You can also reply to request removal from beta emails.\n\nChipTech LLC\nhttps://chiptechllc.com/android-beta/privacy.html`,
  };
  return {
